...
BOOL CGammaRamp::SetBrightness(HDC hDC, WORD wBrightnessR, WORD wBrightnessG, WORD wBrightnessB)
{
	/*
	Changes the brightness of the entire screen.
	This function may not work properly in some video cards.
	...
	*/
	...
	if (hGammaDC != NULL)
	{
		...
		for (int iIndex = 0; iIndex < 256; iIndex++)
		{
			int iArrayValue = iIndex * (wBrightnessR + 128);
			if (iArrayValue > 65535)
				iArrayValue = 65535;
			GammaArray[0][iIndex] = (WORD)iArrayValue;

			iArrayValue = iIndex * (wBrightnessG + 128);
			if (iArrayValue > 65535)
				iArrayValue = 65535;
			GammaArray[1][iIndex] = (WORD)iArrayValue;

			iArrayValue = iIndex * (wBrightnessB + 128);
			if (iArrayValue > 65535)
				iArrayValue = 65535;
			GammaArray[2][iIndex] = (WORD)iArrayValue;
...
BOOL CGammaRamp::ResetBrightness(HDC hDC)
{

	return SetBrightness(hDC, 128);
}
